...
public class ProfileFragment extends Fragment {
    ...
    private final ItemData[] personalProfileItems = {
            ...
            new ItemData(
                    ...
                    profileItem -> {
                        ...
                        getParentFragmentManager().setFragmentResultListener("syncDataState", getViewLifecycleOwner(), (requestKey, result) -> {
    ...
    private final ItemData[] applicationProfileItems = {
            ...
            new ItemData(
                    ...
                    context -> {
                        ...
                        View dndView = getLayoutInflater().inflate(R.layout.layout_dialog_smart_dnd, null);
                        MaterialSwitch dndSwitch = dndView.findViewById(R.id.switch_enable_smart_dnd);
                        
                        boolean isEnabled = SettingsRepository.canEnableAutoSilent(context);
                        dndSwitch.setChecked(isEnabled);

                        dndSwitch.setOnCheckedChangeListener((compoundButton, isChecked) -> {
                            if (isChecked) {
                                ...
                                if (!hasDndAccess) {
                                    ...
                                    Toast.makeText(context, R.string.dnd_permission_warning, Toast.LENGTH_LONG).show();
                                    Intent intent = new Intent(Settings.ACTION_NOTIFICATION_POLICY_ACCESS_SETTINGS);
                                    context.startActivity(intent);
                                } else if (!canScheduleExact && android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.S) {
                                    dndSwitch.setChecked(false);
                                    Toast.makeText(context, "Exact Alarm permission is required for Smart DND.", Toast.LENGTH_LONG).show();
                                    Intent intent = new Intent(Settings.ACTION_REQUEST_SCHEDULE_EXACT_ALARM);
                                    context.startActivity(intent);
                                } else {
                                    SettingsRepository.setAutoSilentEnabled(context, true);
                                    SettingsRepository.rescheduleDndAlarms(context);
                                }
                            } else {
                                SettingsRepository.setAutoSilentEnabled(context, false);
                                SettingsRepository.clearDndAlarms(context);
                            }
                        });
                        ...
                        new MaterialAlertDialogBuilder(context)
                                .setNegativeButton(R.string.cancel, (dialogInterface, i) -> dialogInterface.cancel())
                                .setView(dndView)
                                .setTitle(R.string.smart_dnd_dialog_title)
                                .show();
                    },
                    null
            ),
    ...
    public void onResume() {
        ...
        getParentFragmentManager().setFragmentResultListener("launchSubFragment", getViewLifecycleOwner(), (requestKey, result) -> {
