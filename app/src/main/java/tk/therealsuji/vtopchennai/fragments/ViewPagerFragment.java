...
import tk.therealsuji.vtopchennai.adapters.CalendarPagerAdapter;
...
public class ViewPagerFragment extends Fragment {
    ...
    private void attachCalendar() {
        CalendarDao calendarDao = this.appDatabase.calendarDao();
        float pixelDensity = this.getResources().getDisplayMetrics().density;

        Bundle arguments = this.getArguments();
        String targetDate = arguments != null ? arguments.getString("target_date") : null;
        ...
        if (targetDate != null) {
            ...
            if (parts.length >= 3) {
                try {
                    tempYear = Integer.parseInt(parts[0]);
        ...
        calendarDao
                ...
                .subscribeOn(Schedulers.single())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(new SingleObserver<List<CalendarDao.MonthYear>>() {
                    @Override
                    public void onSubscribe(@NonNull Disposable d) {
                        compositeDisposable.add(d);
                    }
                    ...
                    @Override
                    public void onSuccess(@NonNull List<CalendarDao.MonthYear> uniqueMonths) {
                        ...
                        tabLayoutMediator = new TabLayoutMediator(tabLayout, viewPager, (tab, position) -> {
                            ...
                            tab.setText(String.format(Locale.ENGLISH, "%s %d", name, my.year).toUpperCase());
                            TooltipCompat.setTooltipText(tab.view, null);
                        });
                        tabLayoutMediator.attach();
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        getParentFragmentManager().setFragmentResultListener("customInsets2", getViewLifecycleOwner(), (requestKey, result) -> {
